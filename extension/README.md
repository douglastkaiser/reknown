...
The delay is enforced centrally for profile HTML, overlay, and image requests—not only between people. Every batch begins at an elevated risk level, assuming the account is already under scrutiny, so its first requests receive additional backoff rather than starting from a nominal condition. Transient/protective responses and suspicious consecutive extraction failures increase that bounded exponential backoff, which recovers only one level at a time after successful requests. Concurrent batches are never allowed—including requests carrying the legacy `force` flag—and pending budget reservations count as active starts so simultaneous messages cannot race the guard. The bounded jitter is load-spreading safety pacing, not simulated clicks, scrolling, navigation, or an attempt to evade LinkedIn controls. Enrichment is also capped at 40 profiles per six-hour session and 100 profiles per UTC day; those counters survive background restarts, and the app's cooldown display reports when another batch is eligible.
